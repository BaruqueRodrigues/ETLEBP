test_that("executa o teste de validação da carga incremental", {
  #Diretório do sqlite
  ...
  ft_dispendio <-data.frame(id_disp = 1:7, #id sqlite
                            chamada = NA, #
                            vlr = rep(100000, 7), #valor_executado
                            dta_inicio = NA,
                            id_item = "ANEEL", #id
  ...
  # )
  #
  # ETLEBP::executa_carga_incremental(df_test, filename)
  #
  # #GABARITO
  # gabarito <-data.frame(id_disp = 9, #id sqlite
  #                           chamada = NA, #
  ...
  # tbl_ft_dispendio <- DBI::dbReadTable(con,"ft_dispendio")
  #
  # #Teste 1 onde não há alteração na base
  # expect_equal(tbl_ft_dispendio, gabarito)


  # Dataframe pra inserir a info na função
  df_test2 <- data.frame(
    id = as.character(c(0001, "ANEEL")),
    fonte_de_dados = "ANEEL",
    data_assinatura = lubridate::ymd("2013-01-01"),
    data_limite = lubridate::ymd("2020-12-31"),
    duracao_dias = 2921,
    titulo_projeto = c("Dados Simulados para o teste automático",
                       "Dados Simulados Carga Incremental"),
    status_projeto = NA,
    valor_contratado = 100000,
    valor_executado_2013_2020 = 100000,
    nome_agente_financiador = "",
  ...
  ETLEBP::executa_carga_incremental(df_test2, filename)

  gabarito2 <- data.frame(id_disp = c(1,2,3,4,5,6,7,
                                      NA,NA,NA,NA,
                                      NA,NA,NA,NA), #id sqlite
                          chamada = NA, #
                          vlr = rep(100000, 15), #valor_executado
                          dta_inicio = NA,
                          id_item = "ANEEL", #id
                          ntz_finan = 0, #natureza_financiamento
                          id_cat2 = 13, #categoria slqite
                          ...
                          id_formnt = 5, #fonte_de_dados
                          mod_finan = 1, #
                          ano = c(2013,2014,2015,2016,2017,2018,2019,
                                  2013,2014,2015,2016,2017,2018,2019,2020)
  )

  gabarito2 <- gabarito2 %>% dplyr::mutate(
    chamada = as.integer(chamada),
    dta_inicio = as.integer(dta_inicio),
    id_finan = as.integer(id_finan),
    id_prop = as.integer(id_prop))
  tbl_ft_dispendio <- DBI::dbReadTable(con,"ft_dispendio")

  #Teste 1 onde não há alteração na base
  expect_equal(tbl_ft_dispendio, gabarito2)
